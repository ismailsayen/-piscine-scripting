...
[[ $# -gt 1 || -z $1 ]] && { echo "Error: expect 1 argument only!" >&2 ; exit 1 ; }
...
do
    ...
    [[ ! (($grade =~ ^[0-9]+$)) || "$grade" -lt 0 || "$grade" -gt 100 ]] && { echo "Error: The grade '$grade' is not a valid input. Only numerical grades between 0 and 100 are accepted." >&2 ; exit 1 ;}
